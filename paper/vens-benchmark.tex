...
\author{Fahed Dorgaa\\\small venslabs \;\textbar\; \code{github.com/venslabs/vens-benchmark} \;\textbar\; DOI \href{https://doi.org/10.5281/zenodo.21775841}{10.5281/zenodo.21775841}}
\date{}
...
\begin{abstract}
\noindent
Vulnerability scanners report hundreds of CVEs ranked by CVSS, but CVSS scores
the flaw, not the system it sits in: the same finding can be urgent on an
internet-facing service and irrelevant in an air-gapped batch job. \code{vens}
re-scores each finding against a short project-context file (exposure, data
sensitivity, criticality, controls, compliance) with an LLM, producing an OWASP
Risk Rating from 0 to 81. For operators this raises a practical question: which
model belongs behind it, and is the API cost worth it? We benchmark eight 2026
cloud models and four local models on the two skills the task composes, reading
a CVE and using the declared context, each against a non-LLM rule engine it has
to beat. Three results stand out. Accuracy is cheap and effectively solved: a
\$0.48 model matches a \$4.12 one. Context use is real but uneven: exposure and
accountability move the score correctly, while a denial-of-service cue is
largely keyword-matched rather than reasoned. And the weakest models, including
every local model we tested, are statistically indistinguishable from a constant
baseline. We give a concrete model-selection table and release the harness, so
any new model can be ranked in an afternoon.
\end{abstract}
...
RQ1 so our accuracy numbers sit next to its published baselines. Marchiori et
al.~\cite{marchiori} study the same reading skill in isolation, asking whether an
LLM can compute a CVSS vector from CVE text and finding embedding models
competitive on the more subjective impact metrics. Databricks'
...
\textbf{\code{gpt-5.5} is dominated}, since \code{sonnet} is both more accurate and
...
Model & MAD & 95\% CI & \$/200 \\
\midrule
claude-sonnet-4-6      & \textbf{0.746} & [0.62, 0.88] & 1.90 \\
gpt-5.5                & 0.751 & --- & 4.12 \\
...
\code{flash-lite} and \code{nano} have a median divergence of
...
jittery (SD 3.19), so run $n\ge3$ and take the median. \code{gemini-2.5-flash-lite}
...
\bibitem{marchiori} F. Marchiori, D. Donadel, and M. Conti. Can LLMs Classify
CVEs? Investigating LLMs Capabilities in Computing CVSS Vectors.
TrustAICyberSec 2025. \url{https://arxiv.org/abs/2504.10713}
\end{thebibliography}
